...
<body class="leading-relaxed">
        ...
        <div class="sidebar w-64 flex-shrink-0 bg-white border-r border-neutral-200">
            <div class="p-6">
               ...
               <nav class="space-y-2">
                    ...
                    <a href="./dados.paciente.php" class="sidebar-menu-item active flex items-center space-x-3 p-3">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div class="dashboard-card bg-white rounded-2xl p-6">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <div class="text-right">
                                <div class="text-2xl font-bold text-neutral-800"><?= $user->altura; ?></div>
                                <div class="text-sm text-neutral-500">Cms</div>
                            </div>
                        </div>
                        <h3 class="font-semibold text-neutral-700 mb-1">Minha Altura</h3>
                        <a href="./avaliacoes.paciente.php">
                        </a>
                    </div>
                    ...
                    <div class="dashboard-card bg-white rounded-2xl p-12 text-1xl">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <div class="p-3 bg-primary-100 rounded-xl">
                                <i class="fas fa-ruler text-primary-600 text-xl"></i>
                                <button onclick="openModal('altura')" class="ml-2 text-primary-600 font-medium">Editar Altura</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="modalAltura" class="fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center z-50">
                        <div class="bg-white rounded-2xl p-8 w-96">
                            <div class="flex justify-between items-center mb-6">
                                <h3 class="text-xl font-bold text-neutral-800">Editar Altura</h3>
                                <button onclick="closeModal('altura')" class="text-neutral-400 hover:text-neutral-600">
                                    <i class="fas fa-times text-xl"></i>
                                </button>
                            </div>

                            <form id="formAltura" method="POST" action="atualizar_altura.php">
                                <div class="mb-6">
                                    <label class="block text-sm font-medium text-neutral-700 mb-2">Nova Altura (cm)</label>
                                    <input
                                        type="number"
                                        id="altura"
                                        name="altura"
    ...
    <div id="modalDadosPessoais" class="fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-2xl p-8 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-xl font-bold text-neutral-800">Editar Dados Pessoais</h3>
                <button onclick="closeModal('dadosPessoais')" class="text-neutral-400 hover:text-neutral-600">
                    <i class="fas fa-times text-xl"></i>
                </button>
            </div>

            <form id="formDadosPessoais" method="POST" action="atualizar_dados_pessoais.php">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="col-span-2">
                        ...
                        <input
                            ...
                            name="nome"
                            value="<?= $user->nome; ?>"
                            class="w-full px-4 py-3 border bg-neutral-200 border-neutral-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                    ...
                    <!-- Celular -->
                    <div>
                        <label class="block text-sm font-medium text-neutral-700 mb-2">Celular *</label>
                        <input
                            type="tel"
                            name="celular"
                            value="<?= $user->celular; ?>"
                    ...
                    <div>
                        ...
                        <select
                            ...
                            <option value="O" <?= $user->genero == 'O' ? 'selected' : ''; ?>>Outro</option>
                        </select>
                    </div>

                    <!-- Endereço -->
                    <div class="col-span-2">
                        <label class="block text-sm font-medium text-neutral-700 mb-2">Endereço *</label>
                        <input
                            type="text"
                            name="endereco"
                            value="<?= $user->endereco; ?>"
                ...
                <div class="flex gap-3 mt-8">
                    <button
                        type="button"
                        onclick="closeModal('dadosPessoais')"
                        class="flex-1 px-4 py-3 border border-neutral-300 text-neutral-700 rounded-xl hover:bg-neutral-50 transition-colors">
                        Cancelar
                    </button>
                    <button
    ...
    <script>
        ...
        function openModal(tipo) {
            if (tipo === 'dadosPessoais') {
                document.getElementById('modalDadosPessoais').classList.remove('hidden');
                document.getElementById('modalDadosPessoais').classList.add('flex');
            } else if (tipo === 'peso') {
                document.getElementById('modalPeso').classList.remove('hidden');
                document.getElementById('modalPeso').classList.add('flex');
            } else if (tipo === 'altura') {
        ...
        function closeModal(tipo) {
            if (tipo === 'dadosPessoais') {
                document.getElementById('modalDadosPessoais').classList.add('hidden');
                document.getElementById('modalDadosPessoais').classList.remove('flex');
            } else if (tipo === 'peso') {
                document.getElementById('modalPeso').classList.add('hidden');
                document.getElementById('modalPeso').classList.remove('flex');
            } else if (tipo === 'altura') {
        ...
        // Atualizar o event listener para fechar modal
        document.addEventListener('click', function(event) {
            if (event.target.id === 'modalDadosPessoais') {
                closeModal('dadosPessoais');
            }
            if (event.target.id === 'modalPeso') {
                closeModal('peso');
            }
        ...
        // Atualizar o event listener do ESC
        document.addEventListener('keydown', function(event) {
            if (event.key === 'Escape') {
                closeModal('dadosPessoais');
                closeModal('peso');
